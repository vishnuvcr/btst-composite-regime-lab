...
def execute_btst(selected, cfg):
    """Strict BTST execution: enter at today's close, exit at next session open unless ATR stop/target is touched."""
    ...
    for date,g in selected.groupby('date'):
        ...
        for _,r in picks.iterrows():
            ...
            stop=entry*(1-sm*atr); target=entry*(1+tm*atr)
            # Daily OHLC cannot reveal whether stop or target was hit first.
            # Use conservative stop-first ordering when both are touched.
    ...
    counts=out.groupby('date').symbol.transform('count').clip(lower=1)
    out['weight']=gross/counts; out['weighted_return']=out.net_return*out.weight
    return out
...
def run(config_path):
    ...
    # Align the supervised label with the actual BTST trade: today's close -> next session open.
    ...
    for fid,fold in enumerate(folds,1):
        ...
        routing,rstats=choose_regimes(vp,families,q,maxpr)
